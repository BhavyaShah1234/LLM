"""Chat tuning SFT -- decoder-only, standard (math chat).

Dataset: devrev-research/MathChatSync-reasoning. Fields verified live: only
`conversations` (a ShareGPT-style list of {"from": "human"|"gpt", "value":
...} turns) -- there is no flat `user`/`assistant` field pair as the
archived version of this script (old/chat_tuning_standard.py) assumed, so
every training example's prompt/response text was empty. That script also
built its SYSTEM/USER/ASSISTANT prompt string with a literal `\\n`
(backslash-n) instead of a real newline (confirmed via `cat -A`), had no
evaluation function, and its --debug_first_batch returned before loading
any data. All four are fixed here. See supervised-finetuning/README.md.

Uses the first human/gpt exchange per conversation (single-exchange
simplification, matching this project's other chat-tuning script in scope --
full multi-turn context modeling is a natural future extension).

Format: SYSTEM: ... \\nUSER: {user}\\nASSISTANT: {response}
Loss: assistant tokens only.

Usage:
    python chat_tuning_standard.py --debug_first_batch --max_samples 20
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    """
    p = argparse.ArgumentParser(description="SFT a decoder-only model for chat tuning (standard, math).")
    ...
    p.add_argument("--output_dir", type=str, default="./output/supervised-finetuning/text/chat-tuning/decoder-only/standard", help="Output directory.")
...
class ChatDataset(Dataset):
    """Torch dataset that formats extracted exchanges into chat prompts.

    Each item is tokenized, with the prompt span masked out of `labels` so
    loss is only computed on the assistant's response.
...
def load_and_prepare_data(args, tokenizer):
    """Load, extract, split, and subsample exchanges into train/eval sets.
    ...
    Returns:
        ...
        two are `ChatDataset` instances and `eval_rows` is the raw list of
        eval-split `(user_msg, asst_msg)` tuples (used later for
        generation-based evaluation).
    """
    print_banner("LOADING DATASET")
    raw = load_dataset(DATASET_NAME, split="train")
    exchanges = [extract_exchange(row["conversations"]) for row in raw]
    ...
    # select_samples (common/data_selection.py) expects a HF Dataset with .shuffle()/.select();
    # here we're working with a plain Python list after filtering out incomplete exchanges, so
    # the same random/first/last semantics are applied manually instead.
    ...
    return ChatDataset(train_rows, tokenizer, args.max_length), ChatDataset(eval_rows, tokenizer, args.max_length), eval_rows
...
def decode_example(example, index, tokenizer):
    ...
    return f"Full text:\n{full_text[:600]}...\n\nAssistant response (loss computed on): {label_text[:400]}"
...
def evaluate_model(model, tokenizer, eval_rows, args):
    ...
    Returns:
        dict: `rouge_l` and `bertscore_f1` (or None if scoring failed).
    """
    ...
    with torch.no_grad():
        for i, (user_msg, asst_msg) in enumerate(eval_rows):
            ...
            references.append(asst_msg.strip() or " ")
...
def main():
    """Run the full chat-tuning SFT pipeline: load, train, evaluate, save, record.
    ...
    print_config(args, "Chat tuning SFT -- decoder-only, standard (math)")

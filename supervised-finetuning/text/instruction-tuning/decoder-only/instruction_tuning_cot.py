"""Instruction tuning SFT -- decoder-only, with CoT (code instructions).

Dataset: domofon/evol-instruct-code-cot-80k (same as
instruction_tuning_no_cot.py). Fields: instruction, thinking, response. Two
fixes vs. the archived version of this script
(old/instruction_tuning_cot.py): (1) it read a nonexistent `output` field
for the final answer (the real field is `response`) -- confirmed via the
same live-schema check as instruction_tuning_no_cot.py; (2) it built its
prompt string with literal `\\n` (backslash-n) instead of a real newline
escape (confirmed via `cat -A` on the archived file), so the model
actually never saw a properly formatted multi-line prompt during training.
This version uses real newlines and the correct `response` field.

Format: ### Instruction: ... ### Input: ... ### Response: <think>{thinking}</think>{response}
Loss: on BOTH the thinking and response tokens.

Usage:
    python instruction_tuning_cot.py --debug_first_batch --max_samples 20
"""
...
SYSTEM_INSTRUCTIONS = [
    "You are an expert software engineer. Complete the following coding task, thinking through your approach first.",
    "You are a helpful coding assistant. Think step by step, then write correct, clean code for the following task.",
    "Act as a senior developer. Reason through the following programming task before solving it.",
    "You are a precise code generator. Think through the task, then complete it.",
    "As a programming assistant, reason through the following task before answering.",
]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output_dir", type=str, default="./output/supervised-finetuning/text/instruction-tuning/decoder-only/cot", help="Output directory.")
...
class InstructionCoTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        response = f"<think>{thinking}</think>{row['response']}"
...
def verify_dataset() -> None:
    ...
    for field in ("instruction", "thinking", "response"):
...
def load_and_prepare_data(args, tokenizer):
    ...
    print("Loss computed on: BOTH thinking and response tokens")
    print()

    train_rows = list(train_raw)
    eval_rows = list(eval_raw)
    return (
        InstructionCoTDataset(train_rows, tokenizer, args.max_length, args.seed),
        InstructionCoTDataset(eval_rows, tokenizer, args.max_length, args.seed),
        eval_rows,
    )
...
def decode_example(example, index, tokenizer):
    ...
    return f"Full text:\n{full_text[:600]}...\n\nResponse (loss on BOTH thinking + response): {label_text[:500]}"

...
def extract_answer(response: str) -> str:
    ...
    Args:
        response (str): Raw "response" field text, expected to contain "Answer: X".
...
def extract_reasoning(response: str) -> str:
    """Extract and clean the chain-of-thought reasoning preceding the answer.

    Strips ``<think>``/``</think>`` tags and truncates to 800 characters.

    Args:
        response (str): Raw "response" field text; everything before "Answer:" is
            treated as reasoning.

    Returns:
        str: Cleaned reasoning text, or a generic fallback sentence if empty.
    """
...
class MCQCoTDataset(Dataset):
    """Tokenized medical-MCQ dataset with chain-of-thought supervision.

    Formats each row as ``### Instruction: ... ### Input: ... ### Response:
    <think>{reasoning}</think>Answer: {X}`` and masks the prompt out of the
    loss, so the loss covers both the reasoning and the final answer tokens.
    ...
    def __getitem__(self, idx):
        """Build a tokenized, loss-masked training example with CoT + answer supervision.
        ...
        Returns:
            dict: ``input_ids``, ``attention_mask``, and ``labels`` (prompt
                tokens masked with -100; reasoning and answer tokens kept).
        """
        row = self.rows[idx]
...
def load_and_prepare_data(args, tokenizer):
    ...
    Returns:
        tuple: ``(train_dataset, eval_dataset, eval_rows)`` where the first two
            are :class:`MCQCoTDataset` instances and ``eval_rows`` is the raw
...
def decode_example(example, index, tokenizer):
    ...
    Returns:
        str: Human-readable dump of the full formatted text and the
            reasoning+answer portion the loss is computed on.
    """
...
def evaluate_model(model, tokenizer, eval_rows, args):
    """Generate CoT + answer completions for the eval rows and score answer accuracy.

    Also tracks how often the model actually emits a ``<think>...</think>``
    block and how long that reasoning is, since CoT usage is not guaranteed
    at inference time even though it was trained on.
    ...
    Returns:
        dict: Accuracy, macro F1, whether CoT was enabled, the fraction of
            eval examples that produced a non-empty CoT block, and the
            average CoT length in words.
    """
...
def main():
    """Run the end-to-end MCQ-with-CoT pipeline: load, train, evaluate, save, record."""

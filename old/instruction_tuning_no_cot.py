#!/usr/bin/env python3
"""
Instruction Tuning without CoT - domofon/evol-instruct-code-cot-80k (final answer only, no CoT)
"""
...
SYSTEM_INSTRUCTIONS = [
    "You are an expert coder. You provide clear and accurate code solutions.",
    "You are a skilled programmer who delivers precise coding answers.",
    "You are a professional software engineer providing direct coding solutions.",
    "You are an experienced developer who solves programming problems efficiently.",
    "You are a coding expert who provides concise and accurate implementations."
]
...
def parse_args():
    ...
    Returns:
        ...
        `setup_model_and_tokenizer`, and `main`.
    """
    ...
    parser.add_argument("--output_dir", type=str, default="./output/instruction_tuning_no_cot")
...
class InstructionNoCoTDataset(Dataset):
    """Torch dataset that formats instruction/input/output examples into an Alpaca-style prompt (final answer only) and masks the prompt out of the loss.

    Attributes:
        data (list): list of `{"instruction", "input", "output"}` examples.
        tokenizer: tokenizer used to encode prompt and full (prompt + output) text.
...
def load_and_prepare_data(args):
    """Load the evol-instruct-code-cot dataset, strip `<think>` tags, and split into train/test.
    ...
    Returns:
        tuple[list, list]: `(train_data, test_data)` lists of `{"instruction", "input", "output"}` dicts.
    """
...
def main():
    """Parse args, load data and model, and run the instruction-tuning-without-CoT training loop end to end."""
    args = parse_args()
    random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)
    print(f"\n{'='*80}\nInstruction Tuning WITHOUT CoT\n{'='*80}")
    if args.debug_first_batch: return
    train_data, test_data = load_and_prepare_data(args)
    model, tokenizer = setup_model_and_tokenizer(args)
    train_dataset = InstructionNoCoTDataset(train_data, tokenizer, args.max_length)
    eval_dataset = InstructionNoCoTDataset(test_data[:100], tokenizer, args.max_length)

#!/usr/bin/env python3
"""VQA WITH CoT - opendatalab/ChartVerse-SFT-1.8M (image+question -> <think>reasoning</think>answer)"""
...
def parse_args():
    """Parse command-line arguments for the CoT VQA fine-tuning run.
    ...
    p.add_argument("--output_dir",type=str,default="./output/vqa_cot")
...
class VQACoTDS(Dataset):
    """Torch dataset formatting chart-VQA examples with CoT reasoning.

    Each item pairs an image with an instruction/question prompt whose
    response is `<think>{reasoning}</think>{answer}`, processed jointly by a
    Qwen2-VL processor. Falls back to a blank white image if none is present,
    and skips to the next index on any processing error.

    Attributes:
        data: Formatted examples with `"question"`, `"answer"`,
            `"reasoning"`, and `"image"` keys.
    ...
    def __init__(self,d,proc,ml):
        ...
        Args:
            d: Formatted examples with `"question"`, `"answer"`,
                `"reasoning"`, and `"image"` keys.
            proc: Qwen2-VL processor used to jointly encode text and image.
            ml (int): Maximum tokenized sequence length.
        """
    ...
    def __getitem__(self,i):
        ...
        try:
            ...
            txt=f"### Instruction:\\nAnswer the question about the image. Think through your visual reasoning.\\n### Input:\\n{it['question']}\\n### Response:\\n<think>{it['reasoning']}</think>{it['answer']}"
...
def load_data(a):
    """Stream and format up to 5000 chart-VQA examples with CoT reasoning.

    Streams `opendatalab/ChartVerse-SFT-1.8M`, normalizes question/answer/
    reasoning/image fields, carves off the last 10% as a test split, and
    optionally truncates to `a.max_samples`.
    ...
    """
    print("\\nLoading: opendatalab/ChartVerse-SFT-1.8M (WITH CoT)")
    ds=load_dataset("opendatalab/ChartVerse-SFT-1.8M",split="train",streaming=True)
    def fmt(e):
        """Normalize a raw VQA example's question, answer, reasoning, and image fields.

        Args:
            e (dict): Raw dataset example with a question field (`query` or
                `question`), an answer field (`answer` or `response`), an
                optional `reasoning` field, and an image field (`image` or
                `chart`).

        Returns:
            dict: `{"question", "answer", "reasoning", "image"}`, with
            `reasoning` defaulted to a generic sentence if absent.
        """
        q=e.get("query",e.get("question",""))
        a=e.get("answer",e.get("response",""))
        r=e.get("reasoning","Analyzing the visual elements to determine the answer.")
        img=e.get("image",e.get("chart",None))
        return {"question":q,"answer":a,"reasoning":r,"image":img}
...
def main():
    """Run the end-to-end CoT VQA fine-tuning pipeline.
    ...
    tr,te=load_data(a)
    m,proc=setup(a)
    trd=VQACoTDS(tr,proc,a.max_length)
    evd=VQACoTDS(te[:50],proc,a.max_length)

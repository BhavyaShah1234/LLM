#!/usr/bin/env python3
"""
Standard MCQ (Multiple Choice Questions) Fine-tuning for Qwen Models
Dataset: araag2/MedMCQA
Format: ### Instruction: ... ### Input: {question + options} ### Response: {answer_letter}
Loss: Only on answer letter token
"""
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./output/mcq_standard")
...
def load_and_prepare_data(args):
    ...
    def format_example(example):
        ...
        answer_letter = answer_map.get(answer_idx, "A")
    ...
    # Select test split properly for HuggingFace Dataset
    if "test" in dataset:
    ...
    if hasattr(test_split, 'select'):
        ...
        test_split = list(test_split)[:test_limit]
    ...
    print(f"Dataset: araag2/MedMCQA")
...
def evaluate_model(model, tokenizer, test_data, args):
    ...
    with torch.no_grad():
        for i, item in enumerate(test_data):
            ...
            predictions.append(answer_map[pred_letter])
    ...
    results = {"accuracy": float(accuracy), "f1_macro": float(f1_macro)}
...
def main():
    ...
    train_dataset = MCQDataset(train_data, tokenizer, args.max_length)
    eval_dataset = MCQDataset(test_data[:100], tokenizer, args.max_length)

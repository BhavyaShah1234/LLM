"""Continued pretraining -- decoder-only (CLM), continuing an EXISTING checkpoint.

Unlike pretraining/clm.py (which trains a model from a random init), this
script loads an already-pretrained checkpoint and continues training it with
the same objective (causal language modelling) on more/different raw text.
Defaults to continuing this project's own from-scratch pretraining/clm.py
output (./output/pretraining/clm) on a *different* general corpus
(wikitext-103, Wikipedia articles) than clm.py was originally trained on
(roneneldan/TinyStories, short synthetic stories) -- so this script's job is
literally "keep training the same model on more general text," the textbook
definition of continued pretraining, as opposed to domain-adaptation.py
(this project's other Wave-3 script), which specializes a model to a
*specific* domain's text distribution instead.

--model also accepts any causal-LM checkpoint (local path or HF Hub id), so
this script works equally well continuing pretraining of a real small model
like Qwen/Qwen3-1.7B-Base -- in which case --quantization/--lora become
relevant the same way they do for the supervised-finetuning/ scripts (full-
parameter continued pretraining of a 1.7B model does not fit in 8GB VRAM on
this project's hardware, confirmed empirically while building
supervised-finetuning/ -- see its README).

Dataset: iohadrubin/wikitext-103-raw-v1 (a namespaced mirror of the classic
wikitext-103 corpus -- the original un-namespaced `wikitext` dataset repo
does not resolve correctly against this project's `datasets==5.0.1` pin,
confirmed empirically). Unlike TinyStories, wikitext-103 is real Wikipedia
prose -- continuing on it teaches the model general encyclopedic language
beyond the simple vocabulary/grammar of the synthetic children's-story
corpus it started on.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--quantization", type=str, default="no", choices=["no", "4bit", "8bit"], help="Quantization for loading (relevant if --model is a larger checkpoint than this project's toy pretrained one). Default: no.")

    p.add_argument("--lora", action="store_true", default=False, help="Enable LoRA instead of full-parameter continued pretraining (relevant for larger --model checkpoints).")
    ...
    p.add_argument("--max_samples", type=int, default=-1, help="Number of raw dataset rows (articles) to use before packing. -1 (default) = use the full split.")
    ...
    p.add_argument("--learning_rate", type=float, default=5e-5, help="Learning rate. Default: 5e-5 (much lower than pretraining/clm.py's 3e-4 -- continuing an already-trained model needs gentler updates to avoid catastrophic forgetting).")
    ...
    p.add_argument("--output_dir", type=str, default="./output/continued-pretraining", help="Where to save the continued-pretrained model, tokenizer, and run_result.json.")
...
def verify_dataset() -> None:
    ...
    print(f"Sample: {example['text'][:150]!r}")
    print()
...
def tokenize_and_pack(dataset, tokenizer, block_size: int, desc: str):
    ...
    def tokenize_fn(examples):
        """Drop rows that are empty after stripping and tokenize the rest.

        Args:
            examples (dict): Batch with a `text` column.

        Returns:
            dict: Tokenizer output (`input_ids`, `attention_mask`, ...) for
            the non-empty rows.
        """
...
def main():
    ...
    try:
        ...
        raise OSError(
            f"Could not load a tokenizer from --model={args.model!r}. If you meant this "
            f"project's own toy pretrained checkpoint, run pretraining/clm.py first (see "
            f"pretraining/README.md); otherwise pass an HF Hub model id."
        ) from e
    ...
    print_banner("MODEL")
    print(f"Continuing from: {args.model}")
